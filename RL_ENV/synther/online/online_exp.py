...
sys.path.append('/mnt/share/lixu/SynthER')
...
def redq_sac(
        ...
        # MaximumDiversity hyperparameters
        MaximumDiversity=-1,  # 'her'  'per'
        vector_selection = 0,
        index_type = 0,
        normalization = 0,
        diffusion_random = 0,
    ...
    #    env_fn = lambda: wrap_gym_2(gym.make(env_name))
    #else:
    ...
    wandb.init(project=project_name, name=logger_kwargs['exp_name'],config={"disable_auto_log": True})
    wandb.config.update(agent_config)
    agent = REDQRLPDAgent(vector_selection ,index_type,normalization,diffusion_random ,diffusion_buffer_size, diffusion_sample_ratio, env_name, obs_dim, act_dim, act_limit, device,
    ...
    for t in range(total_steps):
        ...
        agent.store_data(o, a, r, o2, d,MaximumDiversity)
        if vector_selection!=0:
            print('add_vector_to_index  agent.diversity_index')
            agent.add_vector_to_index(agent.diversity_index,o, a, r, o2, d)
        # let agent update
        agent.train(logger,MaximumDiversity)
        # set obs to next obs
        o = o2
        ep_ret += r
        ...
        if not disable_diffusion and (t + 1) % retrain_diffusion_every == 0 and (t + 1) >= diffusion_start:
            ...
            if vector_selection!=0:
                agent.diversity_diffusion_index.reset()
            ...
            print(f'SimpleDiffusionGenerator Adding {num_samples} samples to replay buffer.')
            Diffusion_samples = []
            for o, a, r, o2, term in zip(observations, actions, rewards, next_observations, terminals):
                ...
                #if vector_selection!=0:
                #    print('add_vector_to_index  agent.diversity_diffusion_index')
                #    agent.add_vector_to_index(agent.diversity_diffusion_index,o, a, r, o2, d)
                if vector_selection!=0:
                    ...
                    Diffusion_samples.append(t_prime[0])
                    if(len(Diffusion_samples)%10000==0):
                        print('Diffusion_samples length:',len(Diffusion_samples))

            if vector_selection!=0:
                Diffusion_samples=np.array(Diffusion_samples)
                agent.diversity_diffusion_index.add(Diffusion_samples)
                print('our diversity_diffusion_index points number:',agent.diversity_diffusion_index.ntotal)
...
def wrap_gym_2(env: gym.Env, rescale_actions: bool = True) -> gym.Env:
    if rescale_actions:
        env = gym.wrappers.RescaleAction(env, -1, 1)
...
if __name__ == '__main__':
    ...
    # 遍历 gin_params 中的所有参数
    for param in args.gin_params:
        key, value = param.split('=')
        exp_name_full+='_'+key+'_'+value
    
    
    if args.seed==-1:
        args.seed=int(time.time()) % 10000
    exp_name_full+='_seed_'+str(args.seed)

    exp_name_full+='_batch_size_'+str(args.batch_size)

    print('exp name: ', exp_name_full)
    #exit(1)
    ...
    redq_sac(args.env, seed=args.seed,
             debug=args.debug, batch_size= args.batch_size, target_entropy='auto', project_name=args.project_name,
